...
if(isset($_POST['submit'])){
   ...
   $select = $conn->prepare("SELECT * FROM `users` WHERE role IN ('client') and nom = ?");
   $select->execute([$nom]);
   ...
   if($select->rowCount() > 0){
      ...
      if($pass != $cpass){
         ...
         $insert = $conn->prepare("INSERT INTO `users`(nom, prenom, adresse, num_tel, date,  password) VALUES(?,?,?,?,now(),?)");
         $insert->execute([$nom, $prenom, $adresse, $num_tel, $cpass]);
         if($insert){
